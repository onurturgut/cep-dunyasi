...
import { sendSmtpEmail, isSmtpEmailConfigured } from "@/server/services/smtp-email";
export { isSmtpEmailConfigured } from "@/server/services/smtp-email";

const PASSWORD_RESET_CODE_TTL_MS = 1000 * 60 * 10;
const PASSWORD_RESET_RESEND_COOLDOWN_MS = 1000 * 30;
...
export function getPasswordResetRetrySeconds(sentAt?: Date | string | null) {
  if (!sentAt) {
    return 0;
  }

  const sentAtTime = new Date(sentAt).getTime();
  const remainingMs = sentAtTime + PASSWORD_RESET_RESEND_COOLDOWN_MS - Date.now();

  return remainingMs > 0 ? Math.ceil(remainingMs / 1000) : 0;
}

export function getPasswordResetExpiresInSeconds(expiresAt?: Date | string | null) {
  if (!expiresAt) {
    return 0;
  }
...
function formatResetCode(code: string) {
...
export async function sendPasswordResetCodeEmail(input: {
  ...
  const visualCode = formatResetCode(input.code);
  const escapedCode = escapeHtml(visualCode);

  return sendSmtpEmail({
    to: input.email,
    subject: "Sifre sifirlama kodunuz",
    text: `${customerName}, şifrenizi sıfırlamak için kullanacağınız kod: ${visualCode}. Kod 10 dakika boyunca geçerlidir.`,
    html: `
      <div style="font-family:Arial,sans-serif;line-height:1.6;color:#0f172a;padding:24px;background:#f8fafc">
        <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #e2e8f0;border-radius:24px;padding:32px">
          ...
          <h1 style="margin:0 0 12px;font-size:28px;color:#0f172a">Şifre sıfırlama kodunuz hazır</h1>
          <p style="margin:0 0 20px;color:#334155">${escapedName}, aşağıdaki 6 haneli kodu şifre yenileme ekranına girerek yeni şifrenizi oluşturabilirsiniz.</p>

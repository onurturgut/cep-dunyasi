  ...
  return (
    <Card className="rounded-[1.75rem] border-border/70 shadow-[0_18px_40px_-36px_rgba(15,23,42,0.45)]">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {request.request_type === "return" ? "Iade Talebi" : "Degisim Talebi"} - {formatDate(request.created_at)}
            </p>
          </div>
        </div>
      ...
      <CardContent className="space-y-4 p-5 text-sm text-muted-foreground">
        {request.variant_info ? <p><span className="font-medium text-foreground">Varyant:</span> {request.variant_info}</p> : null}
        ...
        {request.admin_note ? (
          <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4">
            <p className="font-medium text-foreground">Magaza notu</p>

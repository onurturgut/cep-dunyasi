        ...
        <div className="rounded-[2rem] border border-slate-200/80 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.92),transparent_42%),linear-gradient(135deg,rgba(248,250,252,0.98),rgba(226,232,240,0.96))] p-4 shadow-[0_24px_70px_-40px_rgba(148,163,184,0.5)] sm:p-6 lg:rounded-[2.5rem] lg:p-8 dark:border-white/[0.08] dark:bg-[radial-gradient(circle_at_top,rgba(148,163,184,0.1),transparent_34%),linear-gradient(135deg,rgba(24,33,49,0.98),rgba(11,18,32,0.98))] dark:shadow-[0_34px_90px_-44px_rgba(2,6,23,0.88)]">
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1.05fr)_minmax(320px,0.95fr)] lg:items-center">
            <div className="order-2 flex flex-col justify-center text-center lg:order-1 lg:text-left">
              ...
              <h2 className="mt-4 font-display text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl dark:text-slate-50">
                {featuredSlide.title}
              </h2>
            ...
            <div className="order-1 rounded-[1.75rem] border border-slate-200/80 bg-white/80 p-5 shadow-[0_18px_44px_-30px_rgba(148,163,184,0.7)] dark:border-white/[0.08] dark:bg-[linear-gradient(180deg,rgba(24,33,49,0.96),rgba(16,23,37,0.96))] dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] lg:order-2">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {promoCards.map((card) => (
              <article
                key={card.id}
                className="rounded-[1.75rem] border border-slate-200/80 bg-white/80 p-5 shadow-[0_18px_40px_-30px_rgba(148,163,184,0.65)] dark:border-white/[0.08] dark:bg-[linear-gradient(180deg,rgba(24,33,49,0.96),rgba(16,23,37,0.96))] dark:shadow-[0_24px_48px_-36px_rgba(2,6,23,0.86)]"
              >
                <div className="flex items-start justify-between gap-4">
                  {card.badgeText ? <CampaignBadge themeVariant={card.themeVariant}>{card.badgeText}</CampaignBadge> : <span />}
                </div>

                <div className="mt-4 rounded-[1.5rem] border border-slate-200/70 bg-slate-50 px-4 py-6 dark:border-white/[0.08] dark:bg-[#2b3140]">
                  <Image
                ...
                <h3 className="mt-4 font-display text-xl font-semibold tracking-tight text-slate-950 dark:text-slate-50">
                  {card.title}
                </h3>
